...
MatrixXd state1::predict_sigma(const MatrixXd& augmented_sigma, double dt) {
	...
	for (int c = 0; c < _NSIGMA; ++c) {
		...
		*/
		...
		J_e << -m1.q.x(), m1.q.w(), -m1.q.z(), m1.q.y(), -m1.q.y(), m1.q.z(), m1.q.w(), -m1.q.x(), -m1.q.z(), -m1.q.y(), m1.q.x(), m1.q.w();
		Vector4d qdiff = Eigen::Vector4d(m1.q.w() - m1.prev_q.w(), m1.q.x() - m1.prev_q.x(), m1.q.y() - m1.prev_q.y(), m1.q.z() - m1.prev_q.z());
		...
		Vector3d new_pos = (AngleAxisd(w.norm() * dt, w.normalized()).toRotationMatrix() * u) + m1.h;
...
ukf1::ukf1(ALL_DATA, double t, bool debug) : ukf() {
	...
	//vector<double> noises{ VAR_PX_NOISE, VAR_PY_NOISE, VAR_PZ_NOISE, VAR_VYAW_NOISE, VAR_VPITCH_NOISE, VAR_VROLL_NOISE, VAR_DIST2CENTER_NOISE };
